...
这里是一个测试:你觉得例 2-1 和例 2-2 哪个更容易读懂？
...
任何懂一点 Python 的人都可以阅读示例 2-1 。然而，在了解了 listcomps 之后，我发现例 2-2 更具可读性，因为它的意图很明确。
...
listcomps做`map`和`filter`函数所做的一切，而不会扭曲 Python `lambda`的功能。考虑例 2-3 。
...
例 2-5 展示了 genexps 构建元组和数组的基本用法。
...
例 2-6 用一个带笛卡尔积的 genexp 打印出一份两种颜色三种尺寸的 T 恤衫的花名册。与例 2-4 相比，这里的 t 恤六件商品列表从未在内存中构建:生成器表达式一次生成一件商品，并送入`for`循环。如果笛卡尔积中使用的两个列表各有 1000 个条目，那么使用生成器表达式可以节省构建一个包含 100 万个条目的列表来填充`for`循环的成本。
...
例 2-7 显示了用作记录的元组。注意，在每个表达式中，对元组进行排序会破坏信息，因为每个字段的含义是由其在元组中的位置给出的。
...
| ^(a](ch02.xhtml#idm46582502610224-marker)) 反转运算符在[第十六章](ch16.xhtml#operator_overloading)中有解释。^([b](ch02.xhtml#idm46582502593888-marker)) 也用于覆盖一个子序列。参见[“分配给切片”。 |
...
作为解构的第一个例子，例 2-10 展示了用`match/case`改写的例 2-8 的一部分。
...
例如例 2-10 中的`[name, _, _, (lat, lon)]`模式匹配一个四项序列，最后一项必须是两项序列。
...
以`if`开头的可选 guard 子句只有在模式匹配时才会被求值，并且可以引用模式中绑定的变量，如例 2-10 所示:
...
实施例 2-10 不是对实施例 2-8 的改进。这只是一个对比做同一件事的两种方法的例子。下一个例子展示了模式匹配如何有助于清晰、简洁和有效的代码。
...
例 2-11 显示了 Norvig 的赋值器，略有改动，缩写后仅显示序列模式。
...
在 Python ≥ 3.10 中使用`match/case`，我们可以重构`evaluate`如例 2-12 所示。
...
Norvig 故意避免在 *lis.py* 中进行错误检查，以使代码易于理解。有了模式匹配，我们可以添加更多的检查，并且仍然保持它的可读性。例如，在`'define'`模式中，原始代码不能确保`name`是`Symbol`的实例——这需要一个`if`块、一个`isinstance`调用和更多代码。实施例 2-12 比实施例 2-11 更短更安全。
...
有时我们需要用一定数量的嵌套列表初始化一个列表——例如，将学生分配到一个团队列表中，或者在游戏棋盘上表示方块。这样做的最好方法是列表理解，如例 2-14 所示。
...
不使用控制台试着回答:对例 2-16 中的两个表达式求值的结果是什么？ ^([9](ch02.xhtml#idm46582507624544))
...
我看到这个的时候很确定答案是 B，其实是 D，“A 和 B 都有”！示例 2-17 是 Python 3.9 控制台的实际输出。 ^([10](ch02.xhtml#idm46582507593808))
...
分配`sa] = TOS`。如果`s`是不可变的(例 2-17[中的`t`元组)，这将失败。
...
作为一个非常简短的 NumPy 演示，例 2-22 展示了二维数组的一些基本操作。
