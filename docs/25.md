...
[例 21-1](#blogdom_ex) 显示了 *blogdom.py* 的代码。
...
[![5](Images/5.png)](#co_asynchronous_programming_CO2-5)
...
[例 21-11](#web_mojifinder_ex) 没有对`asyncio`的直接调用。 *FastAPI* 构建在 *Starlette* ASGI 工具包上，后者反过来使用`asyncio`。
...
[![1](Images/1.png)](#co_asynchronous_programming_CO16-1)
...
由于有了 *domainlib* ，所以 *domaincheck.py* 的代码很简单，如[示例 21-19](#domaincheck_ex) 所示。
...
[![1](Images/1.png)](#co_asynchronous_programming_CO19-1)
...
[示例 21-21](#blogdom_curio_ex) 显示了 *blogdom.py* 脚本([示例 21-1](#blogdom_ex) )被改写为使用*古董*。

##### 例 21-21。 blogdom.py: [例 21-1](#blogdom_ex) ，现在使用*古玩*
...
[![1](Images/1.png)](#co_asynchronous_programming_CO20-1)

`probe`不需要得到事件循环，因为…

[![2](Images/2.png)](#co_asynchronous_programming_CO20-2)

… `getaddrinfo`是`curio.socket`的顶层函数，而不是`loop`对象的方法——就像在`asyncio`中一样。

[![3](Images/3.png)](#co_asynchronous_programming_CO20-3)

一个`TaskGroup`是 *Curio* 中的核心概念，用来监视和控制几个协程，并确保它们都被执行和清理。

[![4](Images/4.png)](#co_asynchronous_programming_CO20-4)

`TaskGroup.spawn`是如何启动一个协同程序，由一个特定的`TaskGroup`实例管理。协程由一个`Task`包装。

[![5](Images/5.png)](#co_asynchronous_programming_CO20-5)

用`async for`在`TaskGroup`上迭代产生`Task`个实例，因为每个实例都是完成的。这对应于[例 21-1](#blogdom_ex) 中使用 `for … as_completed(…):`的线条。

[![6](Images/6.png)](#co_asynchronous_programming_CO20-6)
...
本章提供了许多带注释的本机协程的例子，包括来自[的`probe`例子 21-21](#blogdom_curio_ex) :
...
为了理解[表 21-1](#latency_tbl) ，请记住，具有 GHz 时钟的现代 CPU 每秒运行数十亿个周期。假设一个 CPU 每秒运行 10 亿个周期。该 CPU 可以在 1 秒钟内进行超过 3.33 亿次 L1 缓存读取，即 4(4！)网络同时读取。[表 21-1 的第三列](#latency_tbl)通过将第二列乘以一个常数因子来显示这些数字。因此，在另一个世界中，如果从 L1 缓存读取一次需要 3 秒，那么网络读取将需要 7.6 年！

[表 21-1](#latency_tbl) 解释了为什么一个训练有素的异步编程方法可以带来高性能的服务器。挑战在于实现这一原则。第一步是认识到“I/O 绑定系统”是一个幻想。

## I/O 绑定系统的神话

一个经常重复的迷因是异步编程对“I/O 受限系统”有好处我艰难地认识到没有“I/O 绑定的系统”你可能有 I/O 绑定的*函数*。也许您系统中的绝大多数函数都是 I/O 受限的；也就是说，他们等待 I/O 的时间比处理数据的时间还要长。在等待时，它们将控制权交给事件循环，后者可以驱动其他一些未完成的任务。但是不可避免的是，任何重要的系统都会有一些部分受到 CPU 的限制。即使是微不足道的系统也显示出，在压力下。在[“soap box”](#async_soapbox)，中，我讲述了两个异步程序的故事，这两个程序与 CPU 绑定的函数进行斗争，减慢了事件循环，严重影响了性能。
...
最后，在[“Async 如何工作以及如何不工作”](#how_async_works_and_does_not_sec)下的部分讨论了异步编程的主要吸引力，对“I/O 绑定系统”的误解，以及处理程序中不可避免的 CPU 绑定部分。
...
^([3](ch21.xhtml#idm46582386163264-marker)) 这个规则有一个例外:如果你用`-m asyncio`选项运行 Python，你可以在`>>>`提示符下直接使用`await`来驱动一个本地协程。这在[“尝试 Python 的异步控制台”](#python_async_console_sec)中有解释。
