...
⑨ `asyncio.as_completed`是一个生成协程的生成器，它按照协程完成的顺序(而不是提交的顺序)返回传递给它的结果。类似于我们在第二十章、例 20-4 中看到的`futures.as_completed`。
...
然而，最终用户代码并不总是需要在`Task`上`await`。我们使用`asyncio.create_task(one_coro())`来调度`one_coro`进行并发执行，而不等待它的返回。这就是我们在 *spinner_async.py* 中对`spinner`协程所做的事情(例 19-4 )。如果不期望取消任务或者等待任务，那么就没有必要保留从`create_task`返回的`Task`对象。创建任务足以调度协程运行。
...
① `get_flag`与例 20-14 中的顺序版本非常相似。第一个区别:它需要`client`参数。
