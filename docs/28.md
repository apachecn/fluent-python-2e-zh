...
③ 类体的其余部分和 *bulkfood_v1.py* ( 例 22-19 )中的原代码一样简单干净。
...
本章的第一个例子是第 22 章中的`LineItem`例子的延续。在例 23-2 中，我们用描述符替换了属性。我们看到描述符是一个提供实例的类，这些实例作为属性部署在托管类中。讨论这种机制需要专门的术语，引入诸如*托管实例*和*存储属性*等术语。
