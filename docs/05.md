...
Python 3 中对 Unicode 的支持已经非常全面和稳定，所以最值得注意的是[“按名称查找字符”](#finding_chars_sec)，描述了一个用于搜索 Unicode 数据库的实用程序——一个从命令行查找带圆圈的数字和微笑的猫的好方法。

值得一提的一个小变化是 Windows 上的 Unicode 支持，从 Python 3.6 开始，它变得更好更简单，我们将在[“小心编码默认值”](#encoding_defaults)中看到。
...
从码位转换成字节是*编码*；从字节转换成码位就是 *解码*。参见[实施例 4-1](#ex_encode_decode) 。
...
[![1](Images/1.png)](#co_unicode_text_versus_bytes_CO1-1)

`str` `'café'`有四个 Unicode 字符。

[![2](Images/2.png)](#co_unicode_text_versus_bytes_CO1-2)

使用 UTF-8 编码对`str`到`bytes`进行编码。

[![3](Images/3.png)](#co_unicode_text_versus_bytes_CO1-3)

`bytes`文字有一个`b`前缀。

[![4](Images/4.png)](#co_unicode_text_versus_bytes_CO1-4)

`bytes` `b`有五个字节(在 UTF-8 中“é”的码位编码为两个字节)。

[![5](Images/5.png)](#co_unicode_text_versus_bytes_CO1-5)
...
`bytes`或`bytearray`中的每一项都是从 0 到 255 的整数，而不是像 Python 2 `str`中的单字符字符串。但是，二进制序列的一个片段总是产生相同类型的二进制序列，包括长度为 1 的片段。参见[实施例 4-2](#ex_bytes_bytearray) 。
...
[![1](Images/1.png)](#co_unicode_text_versus_bytes_CO2-1)

给定一个编码，`bytes`可以从`str`构建。

[![2](Images/2.png)](#co_unicode_text_versus_bytes_CO2-2)

`range(256)`中的每一项都是整数。

[![3](Images/3.png)](#co_unicode_text_versus_bytes_CO2-3)

`bytes`的切片也是`bytes`——甚至是单个字节的切片。

[![4](Images/4.png)](#co_unicode_text_versus_bytes_CO2-4)

`bytearray`没有文字语法:它们显示为`bytearray()`，带有一个`bytes`文字作为参数。

[![5](Images/5.png)](#co_unicode_text_versus_bytes_CO2-5)
...
这就是为什么在[示例 4-2](#ex_bytes_bytearray) 中你会看到`b'caf\xc3\xa9'`:前三个字节`b'caf'`在可打印的 ASCII 范围内，后两个不在。
...
从类似缓冲区的对象构建二进制序列是一个低级操作，可能涉及类型转换。参见[示例 4-3](#ex_buffer_demo) 中的演示。
...
[![1](Images/1.png)](#co_unicode_text_versus_bytes_CO3-1)

Typecode `'h'`创建一个短整数`array`(16 位)。

[![2](Images/2.png)](#co_unicode_text_versus_bytes_CO3-2)

`octets`保存组成`numbers`的字节的副本。

[![3](Images/3.png)](#co_unicode_text_versus_bytes_CO3-3)
...
Python 发行版捆绑了 100 多个 *编解码器*(编码器/解码器)，用于文本到字节的转换，反之亦然。每个编解码器都有一个名称，如`'utf_8'`，通常还有别名，如`'utf8'`、`'utf-8'`和`'U8'`，您可以在`open()`、`str.encode()`、`bytes.decode()`等函数中将其用作`encoding`参数。[示例 4-4](#ex_codecs) 显示了编码为三个不同字节序列的相同文本。
...
[图 4-1](#encodings_demo_fig) 展示了从字母“A”到 G 谱号音乐符号等字符生成字节的各种编解码器。请注意，最后三种编码是可变长度的多字节编码。
...
大多数非 UTF 编解码器只处理一小部分 Unicode 字符。当将文本转换为字节时，如果在目标编码中没有定义字符，将引发`UnicodeEncodeError`，除非通过向编码方法或函数传递一个`errors`参数来提供特殊处理。错误处理程序的行为如[示例 4-5](#ex_encoding) 所示。
...
[![1](Images/1.png)](#co_unicode_text_versus_bytes_CO4-1)

UTF 编码处理任何`str`。

[![2](Images/2.png)](#co_unicode_text_versus_bytes_CO4-2)

`iso8859_1`也适用于`'São Paulo'`字符串。

[![3](Images/3.png)](#co_unicode_text_versus_bytes_CO4-3)

`cp437`无法对`'ã'`(带颚化符的“a”)进行编码。默认的错误处理程序`'strict'`引发`UnicodeEncodeError`。

[![4](Images/4.png)](#co_unicode_text_versus_bytes_CO4-4)

`error='ignore'`处理程序跳过无法编码的字符；这通常是一个非常糟糕的想法，会导致无声的数据丢失。

[![5](Images/5.png)](#co_unicode_text_versus_bytes_CO4-5)

编码时，`error='replace'`用`'?'`替代无法编码的字符；数据也会丢失，但用户会得到一个线索，表明出了问题。

[![6](Images/6.png)](#co_unicode_text_versus_bytes_CO4-6)
...
[示例 4-6](#ex_decoding) 说明了使用错误的编解码器可能会产生小精灵或 `UnicodeDecodeError` 。
...
[![1](Images/1.png)](#co_unicode_text_versus_bytes_CO5-1)

单词“Montréal”编码为`latin1`；`'\xe9'`是表示“é”的字节。

[![2](Images/2.png)](#co_unicode_text_versus_bytes_CO5-2)

用 Windows 1252 解码可以，因为它是`latin1`的超集。

[![3](Images/3.png)](#co_unicode_text_versus_bytes_CO5-3)

ISO-8859-7 是为希腊文设计的，所以`'\xe9'`字节被曲解，并且没有发出错误。

[![4](Images/4.png)](#co_unicode_text_versus_bytes_CO5-4)

KOI8-R 代表俄罗斯。现在`'\xe9'`代表西里尔字母“и”。

[![5](Images/5.png)](#co_unicode_text_versus_bytes_CO5-5)

`'utf_8'`编解码器检测到`octets`不是有效的 UTF-8，并引发`UnicodeDecodeError`。

[![6](Images/6.png)](#co_unicode_text_versus_bytes_CO5-6)
...
要解决这个问题，在文件顶部添加一个神奇的`coding`注释，如[示例 4-7](#ex_ola_mundo) 所示。
...
在 [示例 4-4](#ex_codecs) 中，你可能已经注意到在 UTF-16 编码序列的开头有几个额外的字节。他们又来了:
...
处理文本 I/O 的 最佳实践是“Unicode 三明治”([图 4-2](#unicode_sandwich_fig) )。 ^([5](ch04.xhtml#idm46582487659424)) 这意味着`bytes`应该在输入时尽早解码为`str`(例如，当打开文件进行读取时)。三明治的“填充”是程序的业务逻辑，文本处理只在`str`对象上完成。你不应该在其他处理过程中编码或解码。在输出时，`str`尽可能晚地被编码为`bytes`。大多数 web 框架都是这样工作的，我们在使用它们的时候很少接触`bytes`。例如，在 Django 中，您的视图应该输出 Unicode`str`；Django 自己负责编码对`bytes`的响应，默认使用 UTF-8。
...
考虑[示例 4-8](#ex_cafe_file1) 中的控制台会话。你能发现这个漏洞吗？
...
我在 Python 3.8.1，64 位，Windows 10 (build 18363)上运行了[示例 4-8](#ex_cafe_file1) 。同样的语句在最近的 GNU/Linux 或 macOS 上运行得非常好，因为它们的默认编码是 UTF-8，给人一种一切正常的错觉。如果在打开文件进行写入时省略了 encoding 参数，那么将使用本地默认编码，我们将使用相同的编码正确地读取文件。但是，这个脚本会根据平台，甚至根据同一平台中的区域设置，生成具有不同字节内容的文件，从而产生兼容性问题。

###### 小费

必须在多台机器上或多种场合下运行的代码不应该依赖于编码默认值。当打开文本文件时，总是传递一个显式的`encoding=`参数，因为缺省值可能从一台机器到下一台机器，或者从一天到下一天发生变化。

在[示例 4-8](#ex_cafe_file1) 中一个奇怪的细节是，第一条语句中的`write`函数报告写了四个字符，但是在下一行中读了五个字符。[例 4-9](#ex_cafe_file2) 是例 4-8 的[的延伸版本，解释了那个和其他细节。](#ex_cafe_file1)

##### 例 4-9。对运行在 Windows 上的[示例 4-8](#ex_cafe_file1) 的进一步检查揭示了错误以及如何修复它
...
[![1](Images/1.png)](#co_unicode_text_versus_bytes_CO6-1)

默认情况下，`open`使用文本模式并返回一个带有特定编码的`TextIOWrapper`对象。

[![2](Images/2.png)](#co_unicode_text_versus_bytes_CO6-2)

`TextIOWrapper`上的`write`方法返回写入的 Unicode 字符数。

[![3](Images/3.png)](#co_unicode_text_versus_bytes_CO6-3)

`os.stat`表示文件有 5 个字节；UTF-8 将`'é'`编码为 2 个字节，0xc3 和 0xa9。

[![4](Images/4.png)](#co_unicode_text_versus_bytes_CO6-4)

打开一个没有显式编码的文本文件会返回一个`TextIOWrapper`，其编码被设置为本地的默认值。

[![5](Images/5.png)](#co_unicode_text_versus_bytes_CO6-5)

一个`TextIOWrapper`对象有一个可以检查的编码属性:在本例中是`cp1252`。

[![6](Images/6.png)](#co_unicode_text_versus_bytes_CO6-6)

在 Windows `cp1252`编码中，字节 0xc3 是一个“γ”(带颚化符的 A)，0xa9 是版权标志。

[![7](Images/7.png)](#co_unicode_text_versus_bytes_CO6-7)

用正确的编码打开同一文件。

[![8](Images/8.png)](#co_unicode_text_versus_bytes_CO6-8)

预期的结果:同样的四个 Unicode 字符用于`'café'`。

[![9](Images/9.png)](#co_unicode_text_versus_bytes_CO6-9)

`'rb'`标志以二进制模式打开文件进行读取。

[![10](Images/10.png)](#co_unicode_text_versus_bytes_CO6-10)

返回的对象是`BufferedReader`而不是`TextIOWrapper`。

[![11](Images/11.png)](#co_unicode_text_versus_bytes_CO6-11)
...
几个设置影响 Python 中 I/O 的编码默认值。参见[示例 4-10](#ex_default_encodings) 中的 *default_encodings.py* 脚本。
...
GNU/Linux (Ubuntu 14.04 到 19.10)和 macOS (10.9 到 10.14)上的[示例 4-10](#ex_default_encodings) 的输出是相同的，表明`UTF-8`在这些系统中到处都被使用:
...
然而在 Windows 上，输出是[例 4-11](#ex_default_encodings_ps) 。
...
然而，如果[示例 4-10](#ex_default_encodings) 的输出被重定向到一个文件，如下所示:
...
在 [示例 4-12](#ex_stdout_check) 中，我对 Unicode 文字使用了`'\N{}'`转义，在这里我们在`\N{}`中写入了字符的正式名称。它相当冗长，但是明确而安全:如果名字不存在，Python 会抛出`SyntaxError`——这比写一个可能是错误的十六进制数要好得多，但是你很久以后才会发现。你可能想写一个注释来解释字符代码，所以`\N{}`的冗长很容易被接受。

这意味着像[示例 4-12](#ex_stdout_check) 这样的脚本在打印到控制台时可以工作，但是当输出被重定向到文件时可能会中断。
...
[例 4-12](#ex_stdout_check) 显示`sys.stdout.isatty()`的结果、`sys.​stdout.encoding`的值以及这三个字符:
...
当我在 PowerShell 或*cmd.exe*上运行 *stdout_check.py* 时，它会像[图 4-3](#fig_stdout_check) 中捕获的那样工作。
...
然而，当我将 *stdout_check.py* 的输出重定向到一个文件时，我得到了[图 4-4](#fig_stdout_check_redir) 。
...
[图 4-4](#fig_stdout_check_redir) 演示的第一个问题是`UnicodeEncodeError`提及字符`'\u221e'`，因为`sys.stdout.encoding`是`'cp1252'`——一个没有`INFINITY`字符的代码页。
...
为了总结这个令人恼火的默认编码问题，让我们最后看一下[示例 4-11](#ex_default_encodings_ps) 中的不同编码:
...
如果你用多种语言处理文本，像[示例 4-13](#ex_normeq) 中的`nfc_equal`和`fold_equal`这样的函数对你的工具箱是很有用的补充。
...
要删除`str`中的所有音调符号，可以使用类似[示例 4-14](#ex_shave_marks) 的函数。
...
[![1](Images/1.png)](#co_unicode_text_versus_bytes_CO8-1)

将所有字符分解成基本字符和组合符号。

[![2](Images/2.png)](#co_unicode_text_versus_bytes_CO8-2)

过滤掉所有结合标记。

[![3](Images/3.png)](#co_unicode_text_versus_bytes_CO8-3)

重组所有字符。

[示例 4-15](#ex_shave_marks_demo) 展示了`shave_marks`的几种用法。

##### 例 4-15。使用[例 4-14](#ex_shave_marks) 中`shave_marks`的两个例子
...
[![1](Images/1.png)](#co_unicode_text_versus_bytes_CO9-1)

只有字母“è”、“和”被替换。

[![2](Images/2.png)](#co_unicode_text_versus_bytes_CO9-2)

“έ”和“é”都被替换了。

[示例 4-14](#ex_shave_marks) 中的函数`shave_marks`工作正常，但可能有些过火。通常删除音调符号的原因是为了将拉丁文文本转换为纯 ASCII，但`shave_marks`也会改变非拉丁文字符——如希腊字母——这些字符永远不会因为失去重音符号而变成 ASCII。因此，只有当基本字符是拉丁字母表中的一个字母时，分析每个基本字符并删除附加标记才有意义。这就是[实施例 4-16](#ex_shave_marks_latin) 的作用。

##### 例 4-16。从拉丁字符中删除组合符号的功能(省略导入语句，因为这是[示例 4-14](#ex_shave_marks) 中 simplify.py 模块的一部分)
...
[![1](Images/1.png)](#co_unicode_text_versus_bytes_CO10-1)

将所有字符分解成基本字符和组合符号。

[![2](Images/2.png)](#co_unicode_text_versus_bytes_CO10-2)

当基本字符是拉丁文时，跳过组合符号。

[![3](Images/3.png)](#co_unicode_text_versus_bytes_CO10-3)

否则，保持当前角色。

[![4](Images/4.png)](#co_unicode_text_versus_bytes_CO10-4)

检测新的基本字符，并确定它是否是拉丁语。

[![5](Images/5.png)](#co_unicode_text_versus_bytes_CO10-5)

重组所有字符。

一个更激进的步骤是替换西方文本中常见的符号(例如，弯引号、长破折号、项目符号等)。)转化为`ASCII`当量。这就是[示例 4-17](#ex_asciize) 中函数`asciize`的作用。

##### 例 4-17。将一些西方印刷符号转换成 ASCII 码(这个片段也是来自[示例 4-14](#ex_shave_marks) 的 simplify.py 的一部分)
...
[![1](Images/1.png)](#co_unicode_text_versus_bytes_CO11-1)

为字符到字符的替换建立映射表。

[![2](Images/2.png)](#co_unicode_text_versus_bytes_CO11-2)

构建字符到字符串替换的映射表。

[![3](Images/3.png)](#co_unicode_text_versus_bytes_CO11-3)

合并映射表。

[![4](Images/4.png)](#co_unicode_text_versus_bytes_CO11-4)

`dewinize`不影响`ASCII`或`latin1`文本，只影响`cp1252`中`latin1`的微软附加物。

[![5](Images/5.png)](#co_unicode_text_versus_bytes_CO11-5)

使用`dewinize`并去除变音符号。

[![6](Images/6.png)](#co_unicode_text_versus_bytes_CO11-6)

将 Eszett 替换为“ss”(我们在这里没有使用 case fold，因为我们希望保留案例)。

[![7](Images/7.png)](#co_unicode_text_versus_bytes_CO11-7)

应用 NFKC 规范化将字符与其兼容码位组合在一起。

[实施例 4-18](#ex_asciize_demo) 为使用中的`asciize`。

##### 例 4-18。使用[中`asciize`的两个示例示例 4-17](#ex_asciize)
...
[![1](Images/1.png)](#co_unicode_text_versus_bytes_CO12-1)

`dewinize`替换花引号、项目符号和(商标符号)。

[![2](Images/2.png)](#co_unicode_text_versus_bytes_CO12-2)
...
不同的地区有不同的排序规则，但是在葡萄牙语和许多使用拉丁字母的语言中，重音和西数字在排序时几乎没有区别。 ^([8](ch04.xhtml#idm46582485433232)) 所以“cajá”被排序为“caja”，必须在“caju”之前
...
在安装了`pt_BR.UTF-8`语言环境的 GNU/Linux (Ubuntu 19.10)上运行[示例 4-19](#ex_locale_sort) ，我得到了正确的结果:
...
詹姆斯·陶贝尔，多产的 Django 撰稿人，一定感受到了这种痛苦并创造了[*py uca*](https://fpy.li/4-17)，一个 Unicode 排序算法(UCA)的纯 Python 实现。[示例 4-20](#ex_pyuca_sort) 展示了它的易用性。
...
`unicodedata`模块具有检索角色元数据的功能，其中`unicodedata.name()`返回角色在标准中的正式名称。[图 4-5](#unicodedata_name_fig) 展示了该功能。 ^([10](ch04.xhtml#idm46582457454960))

![Exploring unicodedata.name in the Python console](Images/flpy_0405.png)

###### 图 4-5。探索 Python 控制台中的`unicodedata.name()`。

你可以使用`name()`功能构建应用程序，让用户通过名字搜索角色。[图 4-6](#cf_demo_fig) 展示了 *cf.py* 命令行脚本，该脚本将一个或多个单词作为参数，并列出官方 Unicode 名称中包含这些单词的字符。 *cf.py* 的完整源代码在[示例 4-21](#ex_cfpy) 中。
...
在[示例 4-21](#ex_cfpy) 中，注意`find`函数中的`if`语句，使用`.issubset()`方法快速测试`query`集合中的所有单词是否出现在根据角色名字构建的单词列表中。感谢 Python 丰富的 set API，我们不需要嵌套的`for`循环和另一个`if`来实现这个检查。
...
[![1](Images/1.png)](#co_unicode_text_versus_bytes_CO13-1)

为要搜索的代码点范围设置默认值。

[![2](Images/2.png)](#co_unicode_text_versus_bytes_CO13-2)

`find`接受`query_words`和可选的仅关键字参数来限制搜索范围，以便于测试。

[![3](Images/3.png)](#co_unicode_text_versus_bytes_CO13-3)

将`query_words`转换成一组大写字符串。

[![4](Images/4.png)](#co_unicode_text_versus_bytes_CO13-4)

获取`code`的 Unicode 字符。

[![5](Images/5.png)](#co_unicode_text_versus_bytes_CO13-5)

获取字符的名称，如果代码点未赋值，则为`None`。

[![6](Images/6.png)](#co_unicode_text_versus_bytes_CO13-6)

如果有一个名字，将它分割成一个单词列表，然后检查`query`集合是否是该列表的子集。

[![7](Images/7.png)](#co_unicode_text_versus_bytes_CO13-7)
...
`unicodedata`模块包括检查一个 Unicode 字符是否代表一个数字的函数，如果是，它代表人类的数值——与它的码位号相对。[示例 4-22](#ex_numerics_demo) 展示了`unicodedata.name()`和`unicodedata.numeric()`的用法，以及`str`的`.isdecimal()`和`.isnumeric()`方法。
...
[![1](Images/1.png)](#co_unicode_text_versus_bytes_CO14-1)

`U+0000`格式的代码点。

[![2](Images/2.png)](#co_unicode_text_versus_bytes_CO14-2)

字符集中在长度为 6 的`str`中。

[![3](Images/3.png)](#co_unicode_text_versus_bytes_CO14-3)

如果字符匹配`r'\d'`正则表达式，则显示`re_dig`。

[![4](Images/4.png)](#co_unicode_text_versus_bytes_CO14-4)

如果`char.isdigit()`是`True`则显示`isdig`。

[![5](Images/5.png)](#co_unicode_text_versus_bytes_CO14-5)

如果`char.isnumeric()`为`True`则显示`isnum`。

[![6](Images/6.png)](#co_unicode_text_versus_bytes_CO14-6)

用宽度 5 和 2 个小数位格式化的数值。

[![7](Images/7.png)](#co_unicode_text_versus_bytes_CO14-7)

Unicode 字符名称。

运行[示例 4-22](#ex_numerics_demo) 给你[图 4-7](#numerics_demo_fig) ，如果你的终端字体有所有这些字形。

![Numeric characters screenshot](Images/flpy_0407.png)

###### 图 4-7。显示数字字符及其元数据的 macOS 终端；`re_dig`表示字符匹配正则表达式`r'\d'`。

[图 4-7 第六列](#numerics_demo_fig)是对角色调用`unicodedata.numeric(char)`的结果。这表明 Unicode 知道表示数字的符号的数值。因此，如果您想创建一个支持泰米尔数字或罗马数字的电子表格应用程序，就去做吧！

[图 4-7](#numerics_demo_fig) 显示正则表达式`r'\d'`匹配数字“1”和梵文数字 3，但不匹配其他一些被`isdigit`函数认为是数字的字符。`re`模块对 Unicode 并不了解。PyPI 上新的`regex`模块旨在最终取代`re`，并提供更好的 Unicode 支持。 ^([11](ch04.xhtml#idm46582456966128)) 我们将在下一节回到`re`模块。
...
如果用`bytes`构建正则表达式，那么`\d`、`\w`等模式只匹配 ASCII 字符；相反，如果这些模式被给定为`str`，它们匹配 ASCII 以外的 Unicode 数字或字母。[示例 4-23](#ex_re_demo) 和[图 4-8](#fig_re_demo) 比较`str`和`bytes`模式如何匹配字母、ASCII 数字、上标和泰米尔数字。
...
[![1](Images/1.png)](#co_unicode_text_versus_bytes_CO15-1)

前两个正则表达式属于`str`类型。

[![2](Images/2.png)](#co_unicode_text_versus_bytes_CO15-2)

后两种是`bytes`型。

[![3](Images/3.png)](#co_unicode_text_versus_bytes_CO15-3)

要搜索的 Unicode 文本，包含用于`1729`的泰米尔数字(逻辑行一直延续到右括号标记)。

[![4](Images/4.png)](#co_unicode_text_versus_bytes_CO15-4)

该字符串在编译时与前一个连接在一起(参见[2 . 4 . 2)。*中的字符串文字串联*](https://fpy.li/4-26)(Python 语言参考)。

[![5](Images/5.png)](#co_unicode_text_versus_bytes_CO15-5)

使用`bytes`正则表达式进行搜索需要一个`bytes`字符串。

[![6](Images/6.png)](#co_unicode_text_versus_bytes_CO15-6)

`str`模式`r'\d+'`匹配泰米尔语和 ASCII 数字。

[![7](Images/7.png)](#co_unicode_text_versus_bytes_CO15-7)

`bytes`模式`rb'\d+'`只匹配数字的 ASCII 字节。

[![8](Images/8.png)](#co_unicode_text_versus_bytes_CO15-8)

`str`模式`r'\w+'`匹配字母、上标、泰米尔语和 ASCII 数字。

[![9](Images/9.png)](#co_unicode_text_versus_bytes_CO15-9)

`bytes`模式`rb'\w+'`只匹配字母和数字的 ASCII 字节。

![Output of ramanujan.py](Images/flpy_0408.png)

###### 图 4-8。运行[例 4-23](#ex_re_demo) 中 ramanujan.py 的截图。

[例子 4-23](#ex_re_demo) 是一个简单的例子来说明一点:你可以在`str`和`bytes`上使用正则表达式，但是在第二种情况下，ASCII 范围之外的字节被视为非数字和非单词字符。
...
但是如果您必须处理(也许是修复)不能以那种方式处理的文件名，您可以将`bytes`参数传递给`os`函数以获得`bytes`返回值。这个特性允许你处理任何文件或路径名，不管你能找到多少小精灵。参见[实施例 4-24](#ex_listdir1) 。
...
[![1](Images/1.png)](#co_unicode_text_versus_bytes_CO16-1)

第二个文件名是“π的位数”。txt”(带希腊字母 pi)。

[![2](Images/2.png)](#co_unicode_text_versus_bytes_CO16-2)
