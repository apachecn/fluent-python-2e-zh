...
这个章节是*流畅 Python* 第二版新增的。在[“一个并发的 Hello World”](#concurrent_hello_world)中的 spinner 例子之前在关于 *asyncio* 的章节中。这里对它们进行了改进，并首次展示了 Python 的三种并发方法:线程、进程和本机协程。

除了最初出现在`concurrent.futures`和 *asyncio* 章节中的几个段落之外，其余内容都是新的。

[《多核世界中的 Python》](#py_in_multicore_world_sec)与本书其余部分不同:没有代码示例。我们的目标是提及一些重要的工具，您可能想学习这些工具来实现高性能并发和并行，而这是 Python 标准库所无法实现的。
...
我们先来回顾一下 *spinner_thread.py* 脚本。[例 19-1](#spinner_thread_top_ex) 列出了脚本中的前两个函数，[例 19-2](#spinner_thread_rest_ex) 展示了其余的。
...
[![1](Images/1.png)](#co_concurrency_models_in_python_CO1-1)

这个函数将在一个单独的线程中运行。`done`参数是`threading.Event`的一个实例，一个简单的同步线程的对象。

[![2](Images/2.png)](#co_concurrency_models_in_python_CO1-2)

这是一个无限循环，因为`itertools.cycle`一次产生一个字符，永远循环遍历字符串。

[![3](Images/3.png)](#co_concurrency_models_in_python_CO1-3)

文本模式动画的技巧:用回车 ASCII 控制字符(`'\r'`)将光标移回到行首。

[![4](Images/4.png)](#co_concurrency_models_in_python_CO1-4)

当事件被另一个线程设置时，`Event.wait(timeout=None)`方法返回`True`；如果`timeout`过去，它返回`False`。. 1s 超时将动画的“帧速率”设置为 10 FPS。如果希望微调器运行得更快，请使用较小的超时值。

[![5](Images/5.png)](#co_concurrency_models_in_python_CO1-5)

退出无限循环。

[![6](Images/6.png)](#co_concurrency_models_in_python_CO1-6)

通过用空格覆盖并将光标移回到开头来清除状态行。

[![7](Images/7.png)](#co_concurrency_models_in_python_CO1-7)
...
在[示例 19-2](#spinner_thread_rest_ex) 中列出的`supervisor`功能使用一个`Event`信号通知`spin`功能退出。
...
[![1](Images/1.png)](#co_concurrency_models_in_python_CO2-1)

`supervisor`将返回`slow`的结果。

[![2](Images/2.png)](#co_concurrency_models_in_python_CO2-2)

`threading.Event`实例是协调`main`线程和`spinner`线程活动的关键，下面会进一步解释。

[![3](Images/3.png)](#co_concurrency_models_in_python_CO2-3)

要创建一个新的`Thread`，提供一个函数作为`target`关键字参数，并提供位置参数给`target`作为通过`args`传递的元组。

[![4](Images/4.png)](#co_concurrency_models_in_python_CO2-4)

显示`spinner`对象。输出是`<Thread(Thread-1, initial)>`，其中`initial`是线程的状态——意味着它还没有开始。

[![5](Images/5.png)](#co_concurrency_models_in_python_CO2-5)

启动`spinner`螺纹。

[![6](Images/6.png)](#co_concurrency_models_in_python_CO2-6)

调用`slow`，阻塞`main`线程。同时，辅助线程正在运行微调器动画。

[![7](Images/7.png)](#co_concurrency_models_in_python_CO2-7)

将`Event`标志设置为`True`；这将终止`spin` 功能内的`for`循环。

[![8](Images/8.png)](#co_concurrency_models_in_python_CO2-8)

等到`spinner`线程结束。

[![9](Images/9.png)](#co_concurrency_models_in_python_CO2-9)

运行`supervisor`功能。我编写了单独的`main`和`supervisor`函数，使这个例子看起来更像[例子 19-4](#spinner_async_start_ex) 中的`asyncio`版本。
...
`multiprocessing`包支持在单独的 Python 进程而不是线程中运行并发任务。当您创建一个`multiprocessing.Process`实例时，一个全新的 Python 解释器作为子进程在后台启动。由于每个 Python 进程都有自己的 GIL，这允许您的程序使用所有可用的 CPU 内核——但这最终取决于操作系统调度程序。我们将在[“一个自制的进程池”](#naive_multiprocessing_sec)中看到实际的效果，但是对于这个简单的程序来说，它并没有真正的不同。

本节的重点是介绍`multiprocessing`并展示其 API 模拟了`threading` API，使得简单程序从线程到进程的转换变得容易，如 *spinner_proc.py* ( [例 19-3](#spinner_proc_ex) )所示。
...
[![1](Images/1.png)](#co_concurrency_models_in_python_CO3-1)
...
[![2](Images/2.png)](#co_concurrency_models_in_python_CO3-2)

…迫使我们进口`multiprocessing.synchronize` …

[![3](Images/3.png)](#co_concurrency_models_in_python_CO3-3)

…来编写这个类型提示。

[![4](Images/4.png)](#co_concurrency_models_in_python_CO3-4)

`Process`类的基本用法与`Thread`类似。

[![5](Images/5.png)](#co_concurrency_models_in_python_CO3-5)

`spinner`对象显示为`<Process name='Process-1' parent=14868 initial>`，其中`14868`是运行 *spinner_proc.py* 的 Python 实例的进程 ID。

`threading`和`multiprocessing`的基本 API 是相似的，但是它们的实现非常不同，`multiprocessing`有一个更大的 API 来处理多进程编程增加的复杂性。例如，当从线程转换到进程时，一个挑战是如何在被操作系统隔离并且不能共享 Python 对象的进程之间进行通信。这意味着跨越进程边界的对象必须被序列化和反序列化，这会产生开销。在[示例 19-3](#spinner_proc_ex) 中，唯一跨越进程边界的数据是`Event`状态，它是由位于`multiprocessing`模块下面的 C 代码中的低级操作系统信号量实现的。 ^([10](ch19.xhtml#idm46582392270416))
...
如果我们从`main`函数开始，然后研究`supervisor`，那么 spinner 程序的协程版本更容易理解。这就是[例 19-4](#spinner_async_start_ex) 所示。
...
[![1](Images/1.png)](#co_concurrency_models_in_python_CO4-1)

`main`是这个程序中唯一定义的正则函数——其他的是协程。

[![2](Images/2.png)](#co_concurrency_models_in_python_CO4-2)

`asyncio.run`函数启动事件循环来驱动协程，这将最终启动其他协程。在`supervisor`返回之前，`main`功能将保持锁定。`supervisor`的返回值将是`asyncio.run`的返回值。

[![3](Images/3.png)](#co_concurrency_models_in_python_CO4-3)

本机协程用`async def`定义。

[![4](Images/4.png)](#co_concurrency_models_in_python_CO4-4)

`asyncio.create_task`调度`spin`的最终执行，立即返回`asyncio.Task`的一个实例。

[![5](Images/5.png)](#co_concurrency_models_in_python_CO4-5)

`spinner`对象的`repr`看起来像`<Task pending name='Task-2' coro=<spin() running at /path/to/spinner_async.py:11>>`。

[![6](Images/6.png)](#co_concurrency_models_in_python_CO4-6)

`await`关键字调用`slow`，阻塞`supervisor`，直到`slow`返回。`slow`的返回值将被赋给`result`。

[![7](Images/7.png)](#co_concurrency_models_in_python_CO4-7)

`Task.cancel`方法在`spin`协程中引发了一个`CancelledError`异常，我们将在[示例 19-5](#spinner_async_top_ex) 中看到。

[例 19-4](#spinner_async_start_ex) 展示了运行协程的三种主要方式:
...
现在让我们研究[示例 19-5](#spinner_async_top_ex) 中的`spin`和`slow`协程。
...
[![1](Images/1.png)](#co_concurrency_models_in_python_CO5-1)

我们不需要用于表示`slow`已经完成其在 *spinner_thread.py* 中的工作的`Event`参数([示例 19-1](#spinner_thread_top_ex) )。

[![2](Images/2.png)](#co_concurrency_models_in_python_CO5-2)

使用`await asyncio.sleep(.1)`代替`time.sleep(.1)`，暂停而不阻塞其他协程。看这个例子后的实验。

[![3](Images/3.png)](#co_concurrency_models_in_python_CO5-3)

当在控制这个协程的`Task`上调用`cancel`方法时，`asyncio.CancelledError`被引发。是时候退出循环了。

[![4](Images/4.png)](#co_concurrency_models_in_python_CO5-4)

`slow`协程也使用`await asyncio.sleep`代替`time.sleep`。

### 实验:打破旋转器获得洞察力

我推荐一个实验来了解 *spinner_async.py* 是如何工作的。导入`time`模块，然后转到`slow`协程，用对`time.sleep(3)`的调用替换行`await asyncio.sleep(3)`，如[示例 19-6](#spinner_async_time_sleep_ex) 所示。
...
为了理解发生了什么，回想一下使用`asyncio`的 Python 代码只有一个执行流，除非您已经显式地启动了额外的线程或进程。这意味着在任何时间点只有一个协程执行。并发是通过控制从一个协程传递到另一个协程来实现的。在[示例 19-7](#spinner_async_experiment_ex) 中，让我们关注在提议的实验中`supervisor`和`slow`协程中发生了什么。
...
[![1](Images/1.png)](#co_concurrency_models_in_python_CO6-2)

创建`spinner`任务，最终驱动`spin`的执行。

[![2](Images/2.png)](#co_concurrency_models_in_python_CO6-3)

显示屏显示`Task`处于“待定”状态

[![3](Images/3.png)](#co_concurrency_models_in_python_CO6-4)

`await`表达式将控制权转移给`slow`协程。

[![4](Images/4.png)](#co_concurrency_models_in_python_CO6-1)

`time.sleep(3)`阻滞 3 秒；程序中不会发生其他事情，因为主线程被阻塞了——而且它是唯一的线程。操作系统将继续其他活动。3 秒钟后，`sleep`解锁，`slow`返回。

[![5](Images/5.png)](#co_concurrency_models_in_python_CO6-5)
...
*spinner_thread.py* 和 *spinner_async.py* 的行数几乎相同。`supervisor`功能是这些示例的核心。让我们详细比较一下。[实施例 19-8](#thread_supervisor_ex) 仅列出了实施例 19-2 中的`supervisor`。
...
为了比较，[例 19-9](#asyncio_supervisor_ex) 显示了来自[例 19-4](#spinner_async_start_ex) 的`supervisor`协程。
...
在 线程代码([例 19-1](#spinner_thread_top_ex) )中，可以将`slow`函数中的`time.sleep(3)`调用替换为自己喜欢的库的 HTTP 客户端请求，spinner 会一直旋转下去。这是因为一个设计良好的网络库会在等待网络时释放 GIL。

您还可以将`slow`协同程序中的`asyncio.sleep(3)`表达式替换为`await`，以获得来自设计良好的异步网络库的响应，因为这些库提供了协同程序，在等待网络时将控制权交还给事件循环。同时，旋转器将继续旋转。

对于 CPU 密集型代码，情况就不同了。考虑[示例 19-10](#def_is_prime_ex) 中的函数`is_prime`，如果参数是质数，则返回`True`，否则返回`False`。
...
如果调用 *spinner_async.py* 示例的`slow`协程中的`is_prime(5_000_111_000_222_021)`，spinner 将永远不会出现。当我们将`await asyncio.sleep(3)`替换为`time.sleep(3)`时，效果将与[示例 19-6](#spinner_async_time_sleep_ex) 中的相同:完全不旋转。控制流程将从`supervisor`转到`slow`，然后转到`is_prime`。当`is_prime`返回时，`slow`也返回，`supervisor`恢复，在`spinner`任务被执行之前取消它。程序显示冻结大约 3 秒钟，然后显示答案。T32
...
在本例中，总时间大约是每次检查时间的总和，但它是单独计算的，如[例 19-12](#primes_sequential_ex) 所示。
...
[![1](Images/1.png)](#co_concurrency_models_in_python_CO8-1)

`check`函数(在下一个标注中)返回一个`Result`元组，其中包含`is_prime`调用的布尔值和经过的时间。

[![2](Images/2.png)](#co_concurrency_models_in_python_CO8-2)

`check(n)`调用`is_prime(n)`并计算运行时间以返回一个`Result`。

[![3](Images/3.png)](#co_concurrency_models_in_python_CO8-3)

对于样本中的每个数字，我们调用`check`并显示结果。

[![4](Images/4.png)](#co_concurrency_models_in_python_CO8-4)
...
*procs.py* 中的许多新代码都与设置和使用队列有关。文件的顶部在[例 19-13](#primes_procs_top_ex) 中。

###### 警告

`SimpleQueue`在 Python 3.9 中被添加到`multiprocessing`中。如果你使用的是 Python 的早期版本，你可以用[示例 19-13](#primes_procs_top_ex) 中的`SimpleQueue`替换`Queue`。
...
[![1](Images/1.png)](#co_concurrency_models_in_python_CO9-1)
...
[![2](Images/2.png)](#co_concurrency_models_in_python_CO9-2)

`multiprocessing.queues`有我们需要类型提示的`SimpleQueue`类。

[![3](Images/3.png)](#co_concurrency_models_in_python_CO9-3)

`PrimeResult`包括检查素性的数字。将`n`与其他结果字段放在一起可以简化以后的结果显示。

[![4](Images/4.png)](#co_concurrency_models_in_python_CO9-4)

这是一个`SimpleQueue`的类型别名，`main`函数([示例 19-14](#primes_procs_main_ex) )将使用它向完成工作的进程发送数字。

[![5](Images/5.png)](#co_concurrency_models_in_python_CO9-5)

键入第二个`SimpleQueue`的别名，该别名将在`main`中收集结果。队列中的值将是由要进行素性测试的数字组成的元组，以及一个`Result`元组。

[![6](Images/6.png)](#co_concurrency_models_in_python_CO9-6)

这个类似于 *sequential.py* 。

[![7](Images/7.png)](#co_concurrency_models_in_python_CO9-7)

`worker`获取一个包含待检查数字的队列，另一个用于存放结果。

[![8](Images/8.png)](#co_concurrency_models_in_python_CO9-8)

在这段代码中，我使用数字`0`作为*毒丸*:让工人完成的信号。如果`n`不是`0`，继续循环。 ^([14](ch19.xhtml#idm46582390464160))

[![9](Images/9.png)](#co_concurrency_models_in_python_CO9-9)

调用素性检查并入队`PrimeResult`。

[![10](Images/10.png)](#co_concurrency_models_in_python_CO9-10)

发送回一个`PrimeResult(0, False, 0.0)`让主循环知道这个 worker 已经完成了。

[![11](Images/11.png)](#co_concurrency_models_in_python_CO9-11)

`procs`是并行计算主要检查的进程数。

[![12](Images/12.png)](#co_concurrency_models_in_python_CO9-12)

将待检查的数字排入`jobs`中。

[![13](Images/13.png)](#co_concurrency_models_in_python_CO9-13)

为每个工作线程派生一个子进程。每个子进程将在自己的`worker`函数实例中运行循环，直到它从`jobs`队列中取出一个`0`。

[![14](Images/14.png)](#co_concurrency_models_in_python_CO9-14)

启动每个子进程。

[![15](Images/15.png)](#co_concurrency_models_in_python_CO9-15)

为每个进程排队一个`0`，以终止它们。

现在我们来研究[例 19-14](#primes_procs_main_ex) 中 *procs.py* 的`main`函数。
...
[![1](Images/1.png)](#co_concurrency_models_in_python_CO10-1)

如果没有给出命令行参数，则将进程数设置为 CPU 核心数；否则，创建与第一个参数中给出的一样多的进程。

[![2](Images/2.png)](#co_concurrency_models_in_python_CO10-2)

`jobs`和`results`为[示例 19-13](#primes_procs_top_ex) 中描述的队列。

[![3](Images/3.png)](#co_concurrency_models_in_python_CO10-3)

启动`proc`流程消耗`jobs`和过账`results`。

[![4](Images/4.png)](#co_concurrency_models_in_python_CO10-4)

检索结果并显示它们；`report`在![6](Images/6.png)中定义。

[![5](Images/5.png)](#co_concurrency_models_in_python_CO10-5)

显示检查了多少个数字和总运行时间。

[![6](Images/6.png)](#co_concurrency_models_in_python_CO10-6)

参数是`procs`的编号和发布结果的队列。

[![7](Images/7.png)](#co_concurrency_models_in_python_CO10-7)

循环，直到所有进程都完成。

[![8](Images/8.png)](#co_concurrency_models_in_python_CO10-8)

弄一个`PrimeResult`。在队列块上调用`.get()`,直到队列中有一个项目。也可以使它不阻塞，或者设置一个超时。详见 [`SimpleQueue.get`](https://fpy.li/19-23) 文档。

[![9](Images/9.png)](#co_concurrency_models_in_python_CO10-9)

如果`n`为零，则一个进程退出；增加`procs_done`计数。

[![10](Images/10.png)](#co_concurrency_models_in_python_CO10-10)
...
感谢读者 Michael Albert，他注意到我在早期版本中发布的代码在[示例 19-14](#primes_procs_main_ex) 中有一个 [*竞争条件*](https://fpy.li/19-24) 。竞争条件是一种错误，它可能发生也可能不发生，这取决于并发执行单元执行操作的顺序。如果“A”发生在“B”之前，一切正常；但如果 B 先发生，就会出问题。这就是比赛。
...
*   围绕[图 19-3](#one_possible_architecture_fig) 右侧的消息队列构建的任务队列，提供了一个更高级、更易于使用的 API 来将任务分配给在其他机器上运行的进程。
...
回想一下，我们简单的 *procs.py* ( [示例 19-13](#primes_procs_top_ex) )使用了两个队列:一个用于作业请求，另一个用于收集结果。 *Celery* 和 *RQ* 的分布式架构使用了类似的模式。两者都支持使用 [*Redis*](https://fpy.li/19-50) NoSQL 数据库作为消息队列和结果存储。 *Celery* 还支持其他消息队列，如 *RabbitMQ* 或*亚马逊 SQS* ，以及其他用于存储结果的数据库。
...
GIL 主导了关于 Python 中并发和并行计算的讨论，但是我们不应该高估它的影响。这就是[“多核世界中的 Python”](#py_in_multicore_world_sec)的观点。例如，GIL 不会影响系统管理中的许多 Python 用例。另一方面，数据科学和服务器端开发社区已经围绕 GIL 开展工作，提供符合其特定需求的工业级解决方案。最后两节提到了大规模支持 Python 服务器端应用程序的两个常见元素:WSGI 应用服务器和分布式任务队列。
...
^([21](ch19.xhtml#idm46582389974960-marker)) 将应用程序缓存(由您的应用程序代码直接使用)与 HTTP 缓存进行对比，HTTP 缓存将放置在[图 19-3](#one_possible_architecture_fig) 的顶部边缘，以提供静态资产，如图像、CSS 和 JS 文件。内容交付网络(cdn)提供了另一种类型的 HTTP 缓存，部署在离应用程序最终用户更近的数据中心。
