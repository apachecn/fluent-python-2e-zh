...
[“lis . py 中的模式匹配:案例研究”](#pattern_matching_case_study_sec)是一个新的部分。

我更新了[“context lib 实用程序”](#context_utilities_sec)以涵盖自 Python 3.6 以来添加的`contextlib`模块的一些特性，以及 Python 3.10 中引入的新的带括号的上下文管理器语法。
...
最常见的例子是确保文件对象被关闭。[示例 18-1](#with_file_demo) 是使用`with`关闭文件的详细演示。
...
[![1](Images/1.png)](#co_with__match__and_else_blocks_CO1-1)

`fp`被绑定到打开的文本文件，因为文件的`__enter__`方法返回`self`。

[![2](Images/2.png)](#co_with__match__and_else_blocks_CO1-2)

从`fp`中读取`60` Unicode 字符。

[![3](Images/3.png)](#co_with__match__and_else_blocks_CO1-3)

`fp`变量仍然可用— `with`块不像函数那样定义新的作用域。

[![4](Images/4.png)](#co_with__match__and_else_blocks_CO1-4)

我们可以读取`fp`对象的属性。

[![5](Images/5.png)](#co_with__match__and_else_blocks_CO1-5)

但是我们不能从`fp`中读取更多的文本，因为在`with`块的末尾，调用了`TextIOWrapper.__exit__`方法，它关闭了文件。

[示例 18-1](#with_file_demo) 中的第一个标注提出了一个微妙但重要的观点:上下文管理器对象是在`with`之后评估表达式的结果，但是绑定到目标变量的值(在`as`子句中)是由上下文管理器对象的`__enter__`方法返回的结果。
...
[例 18-2](#looking_glass_demo_1) 展示了一个完全无关紧要的上下文管理器的操作，旨在突出上下文管理器和其`__enter__`方法返回的对象之间的区别。
...
[![1](Images/1.png)](#co_with__match__and_else_blocks_CO2-1)

上下文管理器是`LookingGlass`的一个实例；Python 在上下文管理器上调用`__enter__`，结果被绑定到`what`。

[![2](Images/2.png)](#co_with__match__and_else_blocks_CO2-2)

打印一个`str`，然后是目标变量`what`的值。每个`print`的输出将反向输出。

[![3](Images/3.png)](#co_with__match__and_else_blocks_CO2-3)

现在`with`块结束了。我们可以看到`__enter__`返回的值，保存在`what`中，就是字符串`'JABBERWOCKY'`。

[![4](Images/4.png)](#co_with__match__and_else_blocks_CO2-4)

程序输出不再反转。

[实施例 18-3](#looking_glass_ex) 为`LookingGlass`的实施。
...
[![1](Images/1.png)](#co_with__match__and_else_blocks_CO3-1)

Python 调用`__enter__`，除了`self`之外没有任何参数。

[![2](Images/2.png)](#co_with__match__and_else_blocks_CO3-2)

保持原来的`sys.stdout.write`方法，这样我们以后就可以恢复了。

[![3](Images/3.png)](#co_with__match__and_else_blocks_CO3-3)

猴子补丁`sys.stdout.write`，用我们自己的方法替换它。

[![4](Images/4.png)](#co_with__match__and_else_blocks_CO3-4)

返回`'JABBERWOCKY'`字符串，这样我们就有东西可以放入目标变量`what`中。

[![5](Images/5.png)](#co_with__match__and_else_blocks_CO3-5)

我们对`sys.stdout.write`的替换颠倒了`text`的论点，并调用原来的实现。

[![6](Images/6.png)](#co_with__match__and_else_blocks_CO3-6)

如果一切顺利，Python 用`None, None, None`调用`__exit__`；如果出现异常，三个参数将获得异常数据，如本示例后所述。

[![7](Images/7.png)](#co_with__match__and_else_blocks_CO3-7)

恢复原方法为`sys.stdout.write`。

[![8](Images/8.png)](#co_with__match__and_else_blocks_CO3-8)

如果异常不是`None`并且其类型是`ZeroDivisionError`，则打印一条消息…

[![9](Images/9.png)](#co_with__match__and_else_blocks_CO3-9)

…并返回`True`来告诉解释器异常已被处理。

[![10](Images/10.png)](#co_with__match__and_else_blocks_CO3-10)
...
要详细了解上下文管理器如何工作，请参见[示例 18-4](#looking_glass_demo_2) ，其中 `LookingGlass` 在`with`块之外使用，因此我们可以手动调用其`__enter__`和`__exit__`方法。
...
[![1](Images/1.png)](#co_with__match__and_else_blocks_CO4-1)

实例化并检查`manager`实例。

[![2](Images/2.png)](#co_with__match__and_else_blocks_CO4-2)

调用管理器的`__enter__`方法，并将结果存储在`monster`中。

[![3](Images/3.png)](#co_with__match__and_else_blocks_CO4-3)

`monster`是字符串`'JABBERWOCKY'`。因为通过`stdout`的所有输出都经过我们在`__enter__`中修补的`write`方法，所以`True`标识符出现颠倒。

[![4](Images/4.png)](#co_with__match__and_else_blocks_CO4-4)
...
除了在[示例 18-3](#looking_glass_ex) 之后提到的`redirect_stdout`上下文管理器之外，Python 3.5 中还添加了`redirect_stderr`——它的功能与前者相同，但输出指向`stderr`。
...
一个装饰器，让你从一个简单的生成器函数中构建一个上下文管理器，而不是创建一个类和实现接口。参见[“使用@ context manager”](#using_cm_decorator_sec)。
...
[例 18-5](#looking_glass_gen_ex) 将例 18-3 中的`LookingGlass`类替换为生成器函数。
...
[![1](Images/1.png)](#co_with__match__and_else_blocks_CO5-1)

使用`contextmanager`装饰器。

[![2](Images/2.png)](#co_with__match__and_else_blocks_CO5-2)

保留原来的`sys.stdout.write`方法。

[![3](Images/3.png)](#co_with__match__and_else_blocks_CO5-3)

`reverse_write`可以稍后调用`original_write`因为在其闭包中可用。

[![4](Images/4.png)](#co_with__match__and_else_blocks_CO5-4)

用`reverse_write`替换`sys.stdout.write`。

[![5](Images/5.png)](#co_with__match__and_else_blocks_CO5-5)

生成将绑定到`with`语句的`as`子句中的目标变量的值。当`with`的主体执行时，生成器暂停。

[![6](Images/6.png)](#co_with__match__and_else_blocks_CO5-6)

当控制退出`with`块时，执行在`yield`之后继续；这里恢复了原来的`sys.stdout.write`。

[例 18-6](#looking_glass_gen_demo) 显示了`looking_glass`功能的运行情况。
...
[![1](Images/1.png)](#co_with__match__and_else_blocks_CO6-1)

与[例 18-2](#looking_glass_demo_1) 唯一不同的是上下文管理器的名称:`looking_glass`而不是`LookingGlass`。
...
[例 18-5](#looking_glass_gen_ex) 有一个缺陷:如果在`with`块的体中引发了一个异常，Python 解释器会捕捉它，并在`looking_glass`内部的`yield`表达式中再次引发。但是这里没有错误处理，所以`looking_glass`生成器将终止，而不会恢复原来的`sys.stdout.write`方法，使系统处于无效状态。

[例 18-7](#looking_glass_gen_exc_ex) 增加了对`ZeroDivisionError`异常的特殊处理，使其功能等同于基于类的[例 18-3](#looking_glass_ex) 。

##### 例 18-7。 mirror_gen_exc.py:基于生成器的上下文管理器实现异常处理——与[示例 18-3](#looking_glass_ex) 相同的外部行为
...
[![1](Images/1.png)](#co_with__match__and_else_blocks_CO7-1)

为可能的错误消息创建一个变量；这是相对于[实施例 18-5](#looking_glass_gen_ex) 的第一个变化。

[![2](Images/2.png)](#co_with__match__and_else_blocks_CO7-2)

通过设置错误信息来处理`ZeroDivisionError`。

[![3](Images/3.png)](#co_with__match__and_else_blocks_CO7-3)

取消`sys.stdout.write`的猴子补丁。

[![4](Images/4.png)](#co_with__match__and_else_blocks_CO7-4)
...
[示例 18-8](#looking_glass_gen_deco_demo) 显示了用作装饰器的来自[示例 18-5](#looking_glass_gen_ex) 的`looking_glass`上下文管理器。
...
[![1](Images/1.png)](#co_with__match__and_else_blocks_CO8-1)

`looking_glass`在`verse`的主体运行前后工作。

[![2](Images/2.png)](#co_with__match__and_else_blocks_CO8-2)

这证实了原来的`sys.write`已经恢复。

将[示例 18-8](#looking_glass_gen_deco_demo) 与[示例 18-6](#looking_glass_gen_demo) 进行对比，其中`looking_glass`用作上下文管理器。

标准库之外的`@contextmanager`的一个有趣的现实例子是 Martijn Pieters 的[使用上下文管理器](https://fpy.li/18-11)的就地文件重写。[例 18-9](#inplace_ex) 说明如何使用。
...
[例 18-10](#ex_gcd_scheme) 显示了方案中的一个简单例子。
...
[例 18-10](#ex_gcd_scheme) 展示了三个方案表达式:两个函数定义——`mod`和`gcd`——以及一个对`display`的调用，它将输出`(gcd 18 45)`的结果 9。[例 18-11](#ex_gcd_python) 是 Python 中相同的代码(比递归 [*欧几里德算法*](https://fpy.li/18-14) 的英文解释短)。

##### 例 18-11。同[例 18-10](#ex_gcd_scheme) ，用 Python 编写
...
[示例 18-12](#lis_top_ex) 显示lis . py 的第一行。使用`TypeAlias`和`|`类型的 union 操作符需要 Python 3.10。
...
Norvig 的解析器是 36 行代码，展示了 Python 在处理 S 表达式的简单递归语法方面的强大功能——没有字符串数据、注释、宏和其他标准模式的特性，这些特性使解析变得更加复杂([示例 18-13](#lis_parser_ex) )。
...
该组的主要函数是`parse`，它将一个 S 表达式作为`str`并返回一个`Expression`对象，如[示例 18-12](#lis_top_ex) 中所定义:一个`Atom`或一个`list`可能包含更多的原子和嵌套列表。
...
使用 Python 解释器的术语，`parse`的输出是一个 AST(抽象语法树):模式程序的一个方便的表示，作为形成树状结构的嵌套列表，其中最外面的列表是主干，内部的列表是分支，原子是树叶([图 18-1](#ast_fig) )。
...
`Environment`类扩展了`collections.ChainMap`，添加了一个`change`方法来更新一个链式字典中的值，这个值由`ChainMap`实例保存在一个映射列表中:属性`self.maps`。需要使用`change`方法来支持 Scheme `(set! …)`表单，这将在后面描述；参见[实施例 18-14](#environment_class_ex) 。
...
[![1](Images/1.png)](#co_with__match__and_else_blocks_CO9-1)

读取值时，`Environment`的作用与`ChainMap`相同:从左到右在嵌套映射中搜索键。这就是为什么`outer_env`中的`a`值被`inner_env`中的值遮蔽的原因。

[![2](Images/2.png)](#co_with__match__and_else_blocks_CO9-2)

用`[]`赋值会覆盖或插入新的条目，但总是在第一个映射中，在这个例子中是`inner_env`。

[![3](Images/3.png)](#co_with__match__and_else_blocks_CO9-3)

`env.change('b', 333)`寻找`'b'`键，并在`outer_env`中为其指定一个新值。

接下来是`standard_env()`函数，它构建并返回一个加载了预定义函数的`Environment`，类似于 Python 的始终可用的`__builtins__`模块([示例 18-15](#lis_std_env_ex) )。
...
Norvig 的 REPL (read-eval-print-loop)易于理解，但不便于用户使用(参见[示例 18-16](#ex_lispy_repl) )。如果没有给 *lis.py* 提供任何命令行参数，则`repl()`函数由`main()`调用，该函数在模块末尾定义。在`lis.py>`提示下，我们必须输入正确完整的表达式；如果我们忘了加上一个右括号， *lis.py* 就会崩溃。^(T4110)
...
现在我们可以欣赏诺威格的表情评估工具的妙处了——用`match/case`做得更漂亮了一点。[例 18-17](#ex_evaluate_match) 中的`evaluate`功能采用`parse`和`Environment`制造的`Expression`。
...
`consequence`和`alternative`分支必须是单一表达式。如果一个分支中需要多个表达式，你可以用`(begin exp1 exp2…)`将它们组合起来，作为 *lis.py* 中的一个函数提供——参见[示例 18-15](#lis_std_env_ex) 。
...
[示例 18-18](#test_case_defun) 中的 doctest 定义了一个名为`%`的函数，该函数计算一个百分比并将其添加到`global_env`中。
...
防护罩确保`func_exp`不是[例 18-17](#ex_evaluate_match) 中`['quote', 'if', 'define', 'lambda', 'set!']`的前一项。
...
这个 doctest 上接[例 18-18](#test_case_defun) :它假设`global_env`有一个名为`%`的函数。给`%`的参数是算术表达式，以强调参数在函数被调用之前被评估。
...
[![1](Images/1.png)](#co_with__match__and_else_blocks_CO11-1)

当函数由`lambda`或`define`表单定义时调用。

[![2](Images/2.png)](#co_with__match__and_else_blocks_CO11-2)

保存参数名称、主体表达式和环境供以后使用。

[![3](Images/3.png)](#co_with__match__and_else_blocks_CO11-3)

由`case [func_exp, *args]`条款最后一行的`proc(*values)`调用。

[![4](Images/4.png)](#co_with__match__and_else_blocks_CO11-4)

构建`local_env`映射`self.parms`为局部变量名，给定`args`为值。

[![5](Images/5.png)](#co_with__match__and_else_blocks_CO11-5)

构建一个新的组合`env`，先放`local_env`，然后放`self.env`——定义函数时保存的环境。

[![6](Images/6.png)](#co_with__match__and_else_blocks_CO11-6)

迭代`self.body`中的每个表达式，在组合的`env`中对其求值。

[![7](Images/7.png)](#co_with__match__and_else_blocks_CO11-7)
...
由`|`分隔的 系列模式是一个 [*或-模式*](https://fpy.li/18-25) :如果任何子模式成功，则成功。[“评估数字”](#eval_atom_sec)中的模式是或模式:
...
^([7](ch18.xhtml#idm46582396220528-marker)) 为了使递归迭代实用高效，Scheme 和其他函数式语言实现了*适当的尾调用*。有关这方面的更多信息，请参见[“肥皂盒”](#soapbox_with_match)。
...
^([10](ch18.xhtml#idm46582395540048-marker)) 当我研究 Norvig 的 *lis.py* 和 *lispy.py* 时，我开始了一个名为 [*mylis*](https://fpy.li/18-20) 的 fork，它增加了一些特性，包括一个 REPL，它接受部分 S 表达式并提示继续，类似于 Python 的 REPL 知道我们还没有完成并给出第二个提示(`...`)，直到我们输入一个完整的表达式或语句可以被求值。mylis 也优雅地处理了一些错误，但是仍然很容易崩溃。它远不如 Python 的 REPL 健壮。
