...
我用 Python 编写了`max`，以便更容易看出它的工作原理和重载注释之间的关系(内置的`max`在 C 中)；参见实施例 15-2 。
...
为了解决问题#4051](https://fpy.li/shed4051) ，我编写了[示例 15-3 中的代码。 ^([2](ch15.xhtml#idm46582415940880))

##### 例 15-3。*【mymax . py】*:模块顶部，有导入、定义和重载
...
在这些情况下，输入或者是实现 `SupportsLessThan`、的`LT`类型的独立自变量，或者是这些项的`Iterable`。`max`的返回类型与实际参数或项目相同，正如我们在[“有界类型变量”](ch08.xhtml#bounded_typevar_sec)中看到的。
...
PEP 589——typed dict:带有一组固定键的字典的类型提示解决了这个问题。例 15-4 显示了一个简单的`TypedDict`。
...
让我们探索一下`BookDict`在运行时的行为(例 15-5 )。
...
例 15-6 建立一个有效的`BookDict`并在其上尝试一些操作。这显示了`TypedDict`如何使 Mypy 捕捉错误，如示例 15-7 所示。
...
⑤ 前面的`if`语句阻止`reveal_type(authors)`在运行时被调用。`reveal_type`不是运行时 Python 函数，而是 Mypy 提供的调试工具。这就是为什么它没有`import`的原因。其输出见例 15-7 。

⑥ `demo`函数的最后三行是非法的。它们将导致示例 15-7 中的错误信息。

类型检查 *demo_books.py* 从例 15-6 ，我们得到例 15-7 。
...
如果你正在编写嵌入到微型微控制器中的 MicroPython 代码，你可能会编写一个类似于示例 15-8 中所示的函数。 ^([4](ch15.xhtml#idm46582414555152))
...
例 15-9 显示了一个解析 JSON `str`并返回`BookDict`的函数。
...
现在我们用 Mypy ( 例 15-12 )检查 *demo_not_book.py* 。
...
例 15-13 显示 *demo_not_book.py* 输出无意义，但没有运行时错误。在处理 JSON 数据时使用`TypedDict`并没有提供多少类型安全性。
...
在导入时，Python 读取函数、类和模块中的类型提示，并将它们存储在名为`__annotations__`的属性中。例如，考虑示例 15-14 中的`clip`功能。 ^([12](ch15.xhtml#idm46582413651904))
...
正如你所看到的，所有的类型提示现在都是普通的字符串，尽管在`clip`的定义中它们没有被写成带引号的字符串(例 15-14 )。
...
在 例 13-7](ch13.xhtml#ex_tombola_abc) 中，我们定义了`Tombola` ABC:一个像宾果笼子一样工作的类的接口。来自[示例 13-10](ch13.xhtml#ex_lotto) 的`LottoBlower`类是一个具体的实现。现在我们将研究`LottoBlower`的通用版本，就像在[示例 15-15 中使用的一样。
...
让我们试着用一个通用的`BeverageDispenser`类来模拟自助餐厅场景，这个类可以根据饮料的类型进行参数化。参见实施例 15-18 。
...
为了对自助餐厅中可接受的垃圾桶的规则进行建模，我们需要通过一个使用它的例子来引入“逆变”的概念，如例 15-20 所示。
...
^([8](ch15.xhtml#idm46582413762864-marker)) 我报告了*typeshed*[issue # 5535](https://fpy.li/15-17)“asyncio . base _ events 的错误类型提示。服务器套接字属性。塞巴斯蒂安·里托很快就修好了。然而，我决定保留这个例子，因为它说明了`cast`的一个常见用例，并且我编写的`cast`是无害的。
