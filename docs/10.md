...
例 8-4 是一个比较鸭类型化和名义类型化，以及静态类型检查和运行时行为的傻例子。 ^(6)
...
##### 例 8-17。*mode _ float . py*:`mode`作用于`float`和子类型 ^(13)
...
解决方法是`TypeVar`的另一个可选参数:`bound`关键字参数。它为可接受的类型设置了一个上限。在例 8-18 中，我们有`bound=Hashable`，这意味着类型参数可能是`Hashable`或的任何*子类型。 ^(14)*
...
在第 24 章中，例 24-6 使用了`__flag_unknown_attrs`中的`NoReturn`，这种方法旨在产生一个用户友好且全面的错误信息，然后引发`AttributeError`。

这一章的最后一节是关于位置和变量参数的。

# 仅注释位置参数和变量参数

从例 7-9 中回忆 的`tag`功能。我们最后一次看到它的签名是在“仅位置参数”:
