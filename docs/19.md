...
正如之前承诺的，我们将在第 12 章的`Vector`类上实现几个新的操作符。例 16-1 展示了例 12-16 中已经有的`__abs__`方法，以及新增的`__neg__`和`__pos__`一元运算符方法。

##### 例 16-1。 vector_v6.py:在例 12-16 中增加了一元运算符-和+
...
##### 例 16-10。 vector_v6.py:运算符`+`方法添加到 vector_v5.py ( 例 12-16 )
